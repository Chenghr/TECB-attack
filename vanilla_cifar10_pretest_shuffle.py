...
def set_model_releated(args, logger):
    ...
    # 加载预训练模型以及 delta
    # args.resume = "./results/models/CIFAR10/vanilla/20240803_205102/seed=1_saved_models/"
...
def main(logger, device, args):
    for seed in range(3):
        ...
        vanilla_trainer = VanillaTrainer(model_list)
        vanilla_shuffle_trainer = VanillaTrainer(copied_model_list)
        ...
        for epoch in range(0, args.shuffle_epochs):
            shuffle_train_loss = vanilla_shuffle_trainer.train_shuffle(
            ...
            shuffle_test_loss, _, _ = vanilla_shuffle_trainer.test_mul(
                test_queue, criterion, device, args
            )
            # test clean loss            
            main_task_loss, main_task_acc, _ = vanilla_shuffle_trainer.test_mul(
                train_queue, criterion, device, args
            )
            # test backdoor attack loss
            backdoor_task_loss, backdoor_task_acc, _ = vanilla_shuffle_trainer.test_backdoor_mul(
        ...
        _, top1_acc_origin, top5_acc_origin = vanilla_trainer.test_mul(
            test_queue, criterion, device, args
        )
        _, top1_acc_shuffle, top5_acc_shuffle = vanilla_shuffle_trainer.test_mul(
            test_queue, criterion, device, args
        )
        ...
        _, test_asr_acc_origin, _ = vanilla_trainer.test_backdoor_mul(
            non_target_queue, criterion, device, args, delta, target_label
        )
        _, test_asr_acc_shuffle, _ = vanilla_shuffle_trainer.test_backdoor_mul(
            non_target_queue, criterion, device, args, delta, target_label
        )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--corruption_amp', type=float, default=5, help='amplification of corruption')
    ...
    parser.add_argument('--name', type=str, default='vanilla_cifar10', help='experiment name')
    parser.add_argument('--resume', default='./results/models/CIFAR10/vanilla/20240803_205102/seed=1_saved_models', 
                        type=str, metavar='PATH', help='path to latest checkpoint (default: none)')
    parser.add_argument('--save', default='./results/logs/CIFAR10/pretest_shuffle/vanilla', 
    ...
    parser.add_argument('--c', type=str, default='', help='config file')
