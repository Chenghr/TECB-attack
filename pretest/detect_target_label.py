...
from pretest.identify import identify_potential_target_classes
...
def main(device, dataset="cifar10", poison_ratio=0.01, u=2.0, save_dir="../results/pics/pretest/detect_target_label"):
    # 准备数据
    tsne_data, detect_data, detect_labels, target_label = prepare_data(device, dataset, poison_ratio)
    ...
    # 识别潜在目标类别
    u = 2  # 设置一个合适的阈值
    C = 10 if dataset in ["CIFAR10", "CINIC10L"] else 100
    potential_targets, sorted_log_L  = identify_potential_target_classes(detect_data, detect_labels, C, u)

    print(f"Potential target classes: {potential_targets}")
    print(f"True target label: {target_label}")
    print("\nSorted log L values:")
    for class_id, log_L in sorted_log_L:
        print(f"Class {class_id}: {log_L}")
            
    # savedStdout = sys.stdout
    # txt_name = os.path.join(save_dir, f"detect target label in {dataset} (Poison Ratio: {poison_ratio}).txt")
...
def prepare_data(device, dataset="cifar10", poison_ratio=0.1):
    ...
    Returns:
        tuple: 包含以下四个元素的元组:
            ...
            detect_data: key: 真实标签, value: embedding 列表
            detect_labels: key: 真实标签, value: 预测标签列表
    ...
    detect_data = {i: [] for i in range(num_classes)}
    detect_labels = {i: [] for i in range(num_classes)}
    ...
    with torch.no_grad():
        for batch_idx, (trn_X, trn_y, indices) in enumerate(dataloader):
            ...
            for i, (p, t) in enumerate(zip(pred.squeeze(), target)):
                ...
                detect_data[t.item()].append(output[i].cpu().numpy())
                detect_labels[t.item()].append(p.item())

    # 将列表转换为 numpy 数组
    for k in tsne_data:
        tsne_data[k] = np.array(tsne_data[k])
    for k in detect_data:
        detect_data[k] = np.array(detect_data[k])
    for k in detect_labels:
        detect_labels[k] = np.array(detect_labels[k])

    return tsne_data, detect_data, detect_labels, target_label


def calculate_mahalanobis_distance(X):
    """
    计算数据集X中所有点对之间的平均马哈拉诺比斯距离
    
    参数:
    X: numpy array, 形状为 (n_samples, n_features)
    
    返回:
    average_distance: float, 平均马哈拉诺比斯距离
    """
    # 计算均值向量
