...
def main(logger, device, args):
    ...
    for seed in range(1):
        ...
        for epoch in range(0, args.shuffle_epochs):
            ...
            # test clean loss            
            main_task_loss, _, _ = vfl_shuffle_trainer.test_mul(
                train_queue, criterion, device, args
            )
            # test backdoor attack loss
            backdoor_task_loss, _, _ = vfl_shuffle_trainer.test_backdoor_mul(
        ...
        logger.info(f"Main Task Losses: {metrics.main_task_loss}")
        logger.info(f"Backdoor Attack Losses: {metrics.backdoor_attack_loss}")
...
if __name__ == '__main__':
    ...
    shuffle_args_str = f"Running experiment with args: " + \
        ...
        f"shuffle_label_way={args.shuffle_label_way}, lr={args.lr}, shuffle_epochs={args.shuffle_epochs}"
